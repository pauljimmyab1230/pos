...
export default function HistoryPage() {
  ...
  const filteredDocuments =
    filter.length > 0 ? documents.filter((d) => filter.includes(d.tipo)) : documents;
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <span
                      className={`px-2 py-0.5 text-xs font-medium rounded-full ${
                        tipoColors[doc.tipo] || 'bg-gray-50 text-gray-700'
                      }`}
                    >
            ...
            <div className="p-4 border-b">
              <div className="bg-primary-50 rounded-xl p-4 text-center">
                ...
                <p className="text-sm text-gray-500">
                  {selectedDoc.fechaEmision
                    ? new Date(selectedDoc.fechaEmision).toLocaleDateString('es-PE')
                    : ''}
                </p>
              </div>
            </div>
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="font-medium">
                    {selectedDoc.cliente?.razonSocial ||
                      `${selectedDoc.cliente?.nombres || ''} ${
                        selectedDoc.cliente?.apellidos || ''
                      }`.trim() ||
                      'Sin cliente'}
                  </p>
                  <p className="text-sm text-gray-500">
                  ...
                  >
                    <div className="flex-1">
                      ...
                      <p className="text-xs text-gray-500">
                        {item.cantidad} x S/ {Number(item.precioUnit).toFixed(2)}
                      </p>
                    </div>
